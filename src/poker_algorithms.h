...
namespace poker_algo_new {
    ...
    bool is_flush(array<Card,5> cards) {
        short flush_size = 1;
        Suit c = cards[0].suit;
        for (uint8_t i=1; i<cards.size(); i++){
            if(cards[i].suit == c){
                ++flush_size;
            } else {
                break;
        ...
        return (flush_size == cards.size());
    }

    bool is_straight(array<Card,5> cards) {
        ...
        return (straight_size == cards.size());
    }

    vector<Card> find_flush(array<Card,7> cards) {
        // Returns a vector, either empty or with 5 cards of a flush
    ...
    vector<vector<Card>> find_all_straights(array<Card,7> cards) {
        vector<vector<Card>> all_straight_cards; // max 3 straights of 5 cards
        // If cards are already ordered one of first, second or third card must be part of it
        // Differently from flush, there can be multiple straights
        // Start from lowest card, so straights are ordered based on high card
        for(uint8_t i = 0; i<3; i++){
            vector<Card> straight_cards; 
            straight_cards.push_back(cards[i]);
            short straight_size = 1;
            for (uint8_t j = i+1; j<cards.size(); j++){
                if((straight_size<5) && (cards[j].value == (cards[j-1].value + 1))){
                    ++straight_size;
                    straight_cards.push_back(cards[j]);
                } else {
                    break;
                }
            }
            if(straight_size==5){
                all_straight_cards.push_back(straight_cards);
            }
        }
